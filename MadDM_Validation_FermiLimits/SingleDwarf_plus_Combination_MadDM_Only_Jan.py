...
def Apply_Plot_Properties(vers = 'no', C = ''):
      ...
      plt.text(pos_1, 5*10**(-22), r'Limits Comparison - MadDM v.3.0', size=SIZE-5, ha="left", va="center", color = 'black')
...
def Validate_All_One(flag = '' , Channels = ''):
    ...
    for C in Channels:
      ...
      if C == 'tautau':
          ...
          plt.plot(Masses     , MADDM_6_NoProf     , color = 'red' , linestyle='--'  , label = 'MadDM' , linewidth = 1.5 )

      if C == 'bb':
          #plt.plot(Masses[1:] , MADDM_6[1:]        , color = 'red'  , linestyle='-'   , label = 'MadDM' , linewidth = 1.5 )
          plt.plot(Masses[1:] , MADDM_6_NoProf[1:] , color = 'red' , linestyle='--'  , label = 'MadDM' , linewidth = 1.5 )
...
#      plt.plot(mass_f , lim_f , color = 'lightgray' , linestyle='-' , label = 'Fermi-LAT 2016', linewidth = 1.5 )
      ...
      plt.savefig('PLOTS/All_Validation_MadDM_' + C + '_' + flag + '.png', bbox_inches='tight', dpi = 250 )
...
Validate_All_One(flag = 'Only', Channels = Channels )
