...
def Apply_Plot_Properties(vers = 'no', C = ''):
      ...
      plt.text(pos_1, 5*10**(-22), r'Limits Comparison - oneortwosidedfactor ', size=SIZE-5, ha="left", va="center", color = 'black')
...
def Validate_All_One(flag = '' , Channels = ''):
    ...
    Tab_6 = numpy.load('Numpys/Combined_Limits-Onlybbtautau_Marg.npy').item()
    Tab_6_NoProf = numpy.load('Numpys/Combined_Limits_Twosided_bb_tautau.npy').item()
    ...
    for C in Channels:
      ...
      if C == 'tautau':
          plt.plot(Masses     , MADDM_6            , color = 'red'  , linestyle='-'   , label = 'MadDM oneortwosidedfactor=1'  , linewidth = 1.5 )
          plt.plot(Masses     , MADDM_6_NoProf     , color = 'lime' , linestyle='--'  , label = 'MadDM  oneortwosidedfactor=2' , linewidth = 1.5 )

      if C == 'bb':
          plt.plot(Masses[1:] , MADDM_6[1:]        , color = 'red'  , linestyle='-'   , label = 'MadDM oneortwosidedfactor=1' , linewidth = 1.5 )
          plt.plot(Masses[1:] , MADDM_6_NoProf[1:] , color = 'lime' , linestyle='--'  , label = 'MadDM oneortwosidedfactor=2' , linewidth = 1.5 )


      mass_f , lim_f = Read_Fermi_File(input_file='Fermi-Data/likelihoods/limits_'+C+'.txt', dwarf = 'tot' )
      plt.plot(mass_f , lim_f , color = 'lightgray' , linestyle='-' , label = 'Fermi-LAT 2016', linewidth = 1.5 )


      plt.grid()
      legend = plt.legend(fontsize=11, loc = 'lower right', fancybox = True , ncol = 1)
      frame = legend.get_frame()
      frame.set_edgecolor('gray')

      plt.savefig('PLOTS/All_twosided_' + C + '.png', bbox_inches='tight', dpi = 250 )
