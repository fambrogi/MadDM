...
# ----------------
# JH October 18, 2018:
# Changed null-hypothesis in Fermi_sigmav_lim()
# ----------------
...
# JH added:
def res_tot_dw_alternative(dw_in,pred,marginalize,ll_max_sv):
...
#        ll_null += v['ll_null']
    ...
    ll_null = ll_max_sv  # use ll_max as null-hypothesis!
    pval = compute_pvalue(ll_tot,ll_null)
...
def Fermi_sigmav_lim(mDM, x = '' , dndlogx = '' , dw_in = '' , marginalize = True, sigmavmin=1e-35, sigmavmax=1e-15, step_size_scaling=1.0, cl_val = 0.95):
    ...
    # JH added:
    ll_max_sv = Fermi_sigmav_maxlikelihood(mDM, x, dndlogx , dw_in )
    ...
    # JH changed:
    def find_sigmav(x,pred,dw_in,marginalize,ll_max_sv):
        
        pred_sigma = pred*10**(-x)/sigmav0
        pvalue = res_tot_dw_alternative(dw_in,pred_sigma,marginalize,ll_max_sv)[2]
...
#        print "   ",10**(-x), pvalue
    ...
    result  = res_tot_dw_alternative(dw_in,pred_sigma,marginalize,ll_max_sv)
    p_value = result[2]
...
# JH added:
def Fermi_sigmav_maxlikelihood(mDM, x = '' , dndlogx = '' , dw_in = '' , marginalize = True, sigmavmin=1e-35, sigmavmax=1e-15, step_size_scaling=1.0, cl_val = 0.95):
    ...
    def find_sigmav_2(x,pred,dw_in):
        
        pred_sigma = pred*10**(-x)/sigmav0
        ll_tot = res_tot_dw(dw_in,pred_sigma,False)[0]

        return -2.0*ll_tot


    #DIC = { 'Brent':[] , 'Bounded': [] ,}
    find_sig = lambda x: find_sigmav_2(x,pred,dw_in)
    ...
    res = brute(find_sig,[(brute_range_min,brute_range_max)], Ns=num_steps, full_output=False, finish=fmin)
    sigmav_ul = 10**(-res[0])
    ll_max_sv = -0.5*res[1]  # JH: hope res[1] is correct here
