...
\newcommand{\run}{\texttt{run_\card.dat}}
\newcommand{\param}{\texttt{param_\card.dat}}
...
\
\url{https://github.com/fambrogi/MadDM}
...
*** run_card
1001.0     = ebeam1  
10001.0    = ebeam1  for $m_{\chi_D}$=10 TeV
100001.0    = ebeam1  for $m_{\chi_D}$=100 TeV
\end{verbatim}
...
In Tab. \ref{xsec} the cross sections obtained with different runs are shown.
\begin{table*}[!h]
\centering
...
$\mathbf{m_{\chi_D}} $ & $\mathbf{\chi _D \chi _D \rightarrow WW} $ & $\mathbf{\chi _D \chi _D \rightarrow WW X} $ & $\mathbf{\chi _D \chi _D \rightarrow WWX X }$ & $\mathbf{\chi _D \chi _D \rightarrow WW X X X}$ \\  \toprule 
  ...
  \end{tabular}
  \caption{Cross sections for various processes extracted from the LHE files. The "New" cross sections were computed with $N_{Events}$=10,000, while the "Old" ones with $N_{Events}$=100,000. Need to verify the value 130*. }
  \label{xsec}
\end{table*}
...
Here the spectra for the process $\chi_D \chi_D \rightarrow Y0 \rightarrow FFFF$ are shown for $m_{\chi_D}$ = 1 TeV, compared to the \PPPC and \PPPCew~ spectra. To produce the sample, the EW model was modified adding masses to the light quarks and muons, otherwise there is a problem in \MG~when evaluating the cross sections. I used the value of the muon mass (0.105 GeV) for the light quarks, and 4.5 GeV for the bottom. \\

The model can be found at \url{https://github.com/fambrogi/MadDM/tree/master/EW_Study/EW_Model_FermionMass},
while the complete banner can be found in \url{https://github.com/fambrogi/MadDM/blob/master/EW_Study/Banners/xdxd_Y0_FFFF_1TeV_banner.dat} .
