...
I set, in the param and tun card, I set $m_{\tilde \chi_1 ^0}=$ 1 TeV , $m_{\tilde \chi_1 ^{\pm}}=$ 1.0001 TeV (almost mass degenerate), Ebeam = 1.0001 TeV. 

{\color{blue} I am now doing also 3 and 10 TeV.  At least for the 1 TeV case, the emission of 3 Z bosons seems not necessary since the cross section for zz corrections is already a factor 100 smaller than the tree level. Then, I will add the emission of Higgses, photons and Ws to have the complete EW corrections; however, in the case of the DMsimp\_s\_spin0\_EW model model in Fig. \ref{simp1tev}, there was no significant difference. } 
...
\caption{Emission of Z bosons in the minimal DM model (Wino-like LSP) for $m_{\tilde \chi_1 ^0}=$ 1 TeV. We considered separately the processes  $\tilde \chi _1 ^0  \tilde \chi _1 ^0 \rightarrow w^- w^+$,  $\tilde \chi _1 ^0  \tilde \chi _1 ^0 \rightarrow w^- w^+z$ and  $\tilde \chi _1 ^0  \tilde \chi _1 ^0 \rightarrow w^- w^+ zz$. The corresponding cross section is reported in the legend,and it is used no normalized the spectrum with respect to the tree level process (since Pythia gives normalized spectra, I rescaled each process by multiplying by the ratio of the cross sections), e.g. the curve for the one z emission is rescaled by (tree level)/(one z emission). The lime line is obtained by adding the contribution of each process. Note that the PPPC comparison is just for reference. and the MadDM processes are not complete since the emission of a photons, Higgses and Ws should also be included. Compare with Fig. \ref{simp1tev} for the DMsimp\_s\_spin0\_EW model.}
\end{figure}
