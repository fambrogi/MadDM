...
def Plot(DM, dic_EW = '', dic_noEW = '', CR = '', channel = '' , scaled = False, boson = 'z'):
    ...
    for C in chans:
        ...
        for F,C,L,s,so in zip (FILES, COLORS, LABELS, xsections_rel, xsections_abs):
            ...
            x,y = READ(F)
            x = np.array([ math.pow(10,num) for num in x]) ### NB !!!! this factor 2 is because the EBEAM in pythia card was not 2*DM but only 1*DM !!! So x=Ekin/DM form pythia is actually 2*x
