...
def Apply_Plot_Properties(x_label = True, y_label = True):
      ...
      plt.xlim(0.000000000007, 1)
      plt.ylim(0.00001, 400)
      plt.yscale('log')
      plt.xscale('log')
...
def Plot_Comparison_WW(DM='', DIR = ''):
 ...
 for spec_Tab, spec_Pythia,pr in zip (tab,py8,pretty):
    ...
    e,f = np.loadtxt( DIR + './Data_DMDM_WW_ZZ_'+DM + '/'  + spec_Pythia + '_test1000GeV_ww_lhe.dat', unpack=True )

    y = [ math.pow(10,num) for num in a]
    
    aa = plt.plot(y, b , color = 'black' , linestyle= '-', label = r'$\chi \chi \rightarrow W^+ W^-$ MadDM'      , linewidth = TAB_width-0.6 )
    bb = plt.plot(y, d , color = 'blue'  , linestyle= '-', label = r'$\chi \chi \rightarrow W^+ W^- Z$ MadDM '   , linewidth = TAB_width-0.6 )
    cc = plt.plot(y, f , color = 'cyan' , linestyle= '-', label = r'$\chi \chi \rightarrow W^+ W^- Z Z $ MadDM' , linewidth = TAB_width-0.6 )
    ...
    plt.savefig('PLOTS/'+str(DM)+'EW_CorrectionsComparison_' +  spec_Tab + '.png' , bbox_inches='tight', dpi = 250 )
    plt.close()
        #os.system('mkdir PLOTS')
    #plt.savefig('PLOTS/MadDM_Spectra/' + DM + '_' + spectrum + '_Lepton_Leptons.png', bbox_inches='tight', dpi = 250 )


def Plot_Comparison_WW_100TeV(DM='', DIR = ''):
    # ex_test1000GeV_ww_lhe.dat
 ...
 for spec_Tab, spec_Pythia,pr in zip (tab,py8,pretty):
    ...
    # Data_DMDM_WW_ZZ_10TeV
    a,b = np.loadtxt( DIR + './Data_DMDM_WW_'+DM + '/'  + spec_Pythia + '_test1000GeV_ww_lhe.dat', unpack=True )
    a,c = np.loadtxt( DIR + './Data_DMDM_WW_Z_'+DM + '/'  + spec_Pythia + '_test1000GeV_ww_lhe.dat', unpack=True )
    a,d = np.loadtxt( DIR + './Data_DMDM_WW_H_'+DM + '/'  + spec_Pythia + '_test1000GeV_ww_lhe.dat', unpack=True )

    a,e = np.loadtxt( DIR + './Data_DMDM_WW_ZZ_'+DM + '/'  + spec_Pythia + '_test1000GeV_ww_lhe.dat', unpack=True )
    a,f = np.loadtxt( DIR + './Data_DMDM_WW_WW_'+DM + '/'  + spec_Pythia + '_test1000GeV_ww_lhe.dat', unpack=True )
    a,g = np.loadtxt( DIR + './Data_DMDM_WW_WWorZZorHH_'+DM + '/'  + spec_Pythia + '_test1000GeV_ww_lhe.dat', unpack=True )

    y = [ math.pow(10,num) for num in a]
    
    
    aa = plt.plot(y, b , color = 'black' , linestyle = '-', label = r'$\chi \chi \rightarrow W^+ W^-$ MadDM'      , linewidth = TAB_width-0.6 )
    bb = plt.plot(y, c , color = 'cyan'  , linestyle = '-', label = r'$\chi \chi \rightarrow W^+ W^- Z$ MadDM '   , linewidth = TAB_width-0.6 )
    bb = plt.plot(y, d , color = 'blue'  , linestyle = '-', label = r'$\chi \chi \rightarrow W^+ W^- H$ MadDM '   , linewidth = TAB_width-0.6 )

    cc = plt.plot(y, e , color = 'limegreen' ,  linestyle = '-', label = r'$\chi \chi \rightarrow W^+ W^- Z Z $ MadDM' , linewidth = TAB_width-0.6 )
    dd = plt.plot(y, f , color = 'green' , linestyle = '-', label = r'$\chi \chi \rightarrow W^+ W^- W W $ MadDM' , linewidth = TAB_width-0.6 )
    
    ee = plt.plot(y, g , color = 'gold' , linestyle = '-', label = r'$\chi \chi \rightarrow W^+ W^-   (WW,ZZ,HH)$ MadDM' , linewidth = TAB_width-0.6 )
    ...
    os.system('mkdir PLOTS')
    plt.savefig('PLOTS/'+str(DM)+'EW_RANGE_CorrectionsComparison_' +  spec_Tab + '.png' , bbox_inches='tight', dpi = 250 )
    plt.close()
        #os.system('mkdir PLOTS')
    #plt.savefig('PLOTS/MadDM_Spectra/' + DM + '_' + spectrum + '_Lepton_Leptons.png', bbox_inches='tight', dpi = 250 )
...
Plot_Comparison_WW_100TeV(DM = '100000.0', DIR = '')
