...
C     *     Decay: w+ > vt ta+ DMS<=2 WEIGHTED<=2
C     *     Decay: w- > b t~ DMS<=2 WEIGHTED<=2
C     
      USE DISCRETESAMPLER
      IMPLICIT NONE
C     
...
C     ----------
     ...
     $   *SMALL_WIDTH_TREATMENT)), MDL_WW)
        FK_ZERO = SIGN(MAX(ABS(ZERO), ABS(MDL_MXD*SMALL_WIDTH_TREATMENT)
     $   ), ZERO)
        FK_MDL_WY0 = SIGN(MAX(ABS(MDL_WY0), ABS(MDL_MY0
     $   *SMALL_WIDTH_TREATMENT)), MDL_WY0)
        FK_MDL_WT = SIGN(MAX(ABS(MDL_WT), ABS(MDL_MT
     $   *SMALL_WIDTH_TREATMENT)), MDL_WT)
        FK_ZERO = SIGN(MAX(ABS(ZERO), ABS(MDL_MTA*SMALL_WIDTH_TREATMENT)
     $   ), ZERO)
     ...
     $   , ZERO)
      ...
      CALL FFV2_3(W(1,4),W(1,3),GC_43,MDL_MW, FK_MDL_WW,W(1,5))
      CALL OXXXXX(P(0,5),ZERO,NHEL(5),+1*IC(5),W(1,4))
      CALL IXXXXX(P(0,6),MDL_MT,NHEL(6),-1*IC(6),W(1,3))
      CALL FFV2_3(W(1,3),W(1,4),GC_43,MDL_MW, FK_MDL_WW,W(1,6))
      CALL VVS1_2_3_3(W(1,6),W(1,5),GC_20,GC_102,GC_28,MDL_MY0,
     $  FK_MDL_WY0,W(1,3))
C     Amplitude(s) for diagram number 1
      CALL FFS1_2_0(W(1,1),W(1,2),W(1,3),GC_13,GC_14,AMP(1))
