...
axes = plt.subplot(111)
axes.xaxis.set_minor_formatter(FormatStrFormatter("%.2f"))
plt.hist(ma5_values_cleaned,  bins, histtype="step", color = 'blue',   label = 'MadDM (with ch1 t-chan.)' , density = True)
plt.hist(ma5_values_cleaned_s,  bins, histtype="step", color = 'lime', label = 'MadDM (only z s-chan.)' , density = True)
plt.xlim(0 , )
plt.ylim(0 , 0.001)
plt.ylabel("Normalized Counts /"+str(bins)+'GeV')
