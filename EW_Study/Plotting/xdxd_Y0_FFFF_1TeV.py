...
def Plot(DM, dic_EW = '', dic_noEW = '', CR = '', channel = ''):
    ...
    plt.text(0.000002, 400 ,  r' $\chi _D\chi _D \rightarrow Y0 \rightarrow FFFF \ , \ F=(u,d,c,s,t,b,e,\nu_e,\mu,\nu_{\mu},\tau,\nu_{\tau})$    ' , size=SIZE-5, ha="left", va="center" , color = 'red')
    ...
    EW_L = r'MadDM $\chi _D \chi _D \rightarrow Y0 \rightarrow F F F F $ EW'
    NoEW_L = r'MadDM $\chi _D \chi _D \rightarrow Y0 \rightarrow F F F F $ NoEW'
    ...
    for C in chans:
        ...
        EW   = 'Data_xdxd_Y0_FFFF_1TeV_EW/' + Name_Dic[CR] + '_test1000GeV_ww_lhe.dat'
        NOEW = 'Data_xdxd_Y0_FFFF_1TeV_NoEW/' + Name_Dic[CR] + '_test1000GeV_ww_lhe.dat'
        x,y = READ(EW)
        d = plt.plot(x , y , color = 'slateblue' , linestyle= '-', linewidth = width , label = NoEW_L )
        x,y = READ(NOEW)
        c = plt.plot(x , y , color = 'slateblue' , linestyle= '--', linewidth = width , label = EW_L )

    plt.legend(fancybox = True, loc = 'lower right', fontsize = SIZE- 10)
    ...
    os.system('mkdir PLOTS/xdxd_Y0')

    plt.savefig('PLOTS/xdxd_Y0/' + DM + '_' + CR + '_FFFF_xdxd_'+DM +'.pdf', bbox_inches='tight')
    plt.close()
